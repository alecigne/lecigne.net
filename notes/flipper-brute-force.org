...
Here we'll be writing a script in [[https://en.wikipedia.org/wiki/Common_Lisp][Common Lisp]]. I am not a strong Lisp
programmer, but I just [[file:../themes/lisp.org][love Lisp]] :) The script will be tailored to our
need so I won't bother writing defensive or even clean code.
...
Voilà! The full script and the resulting file are available on GitHub
-- the link points to a specific tag that reflects the state of the
script at the time of this writing, but it might have evolved since :)
